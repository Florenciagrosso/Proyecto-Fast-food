...
Imaginemos que quieres eliminar una orden específica, por ejemplo,
la orden con OrdenID = 10. 
Esta operación eliminaría la orden junto con sus detalles asociados
(deberías asegurarte de eliminar o actualizar cualquier dato relacionado para mantener la integridad referencial).*/
...
SELECT COUNT (OrdenID) FROM Ordenes; -- (9)
...
SELECT CategoriaID, COUNT (ProductoID) AS ProductoporCategoria
FROM Productos
...
/*Pregunta: ¿Cuales son las sucursales con un promedio de Facturación/Ingresos superior a 1000.00 y que minimizan
sus costos en base al promedio de kilómetros recorridos de todas de sus entregas gestionadas? Para un mejor relevamiento,
ordene el listado por el Promedio Km Recorridos.*/

SELECT 
    SucursalID,
    AVG(TotalCompra) AS PromedioFacturacion, -- Calculo el promedio de facturacion con la funcion AVG. AS ES PARA RENOMBRAR
    AVG(KilometrosRecorrer) AS PromedioKmRecorridos -- Calculo el promedio de KM recorridos con la funcion AVG
FROM Ordenes
GROUP BY SucursalID
HAVING AVG(TotalCompra) > 1000.00 -- Utilizo la funcion HAVING para filtrar datos agrupados
ORDER BY PromedioKmRecorridos ASC;  -- Ordeno el resultado por el promedio de kilómetros recorridos en orden ascendente.